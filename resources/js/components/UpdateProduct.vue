            ...
            <div class="col-md-6">
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body border">
                        <vue-dropzone v-model="images" name="images[]" ref="myVueDropzone" id="dropzone" :options="dropzoneOptions"></vue-dropzone>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        /*
        * method for product previous all data
        * */
        setPreviousData(){
            ...
            if(this.product.product_variant) {
                const unique = [...new Set(this.product.product_variant.map(item => item.variant_id))];
                if(unique && unique.length>0){
                    this.product_variant=[];
                    unique.filter(item=>{
                        let dd= this.product.product_variant.filter(item2=>item2.variant_id === item);
                        this.product_variant.push({
                            option: item,
                            tags: [...new Set(dd.map(item => item.variant))],
                        })
                    });
                    this.product_variant_prices=[];
                    this.product.product_variant_prices.filter(item=>{
                        let tt1 = item.product_variant_one?item.product_variant_one.variant+'/':'';
                        let tt2 = item.product_variant_two?item.product_variant_two.variant+'/':'';
                        let tt3 = item.product_variant_three?item.product_variant_three.variant+'/':'';
                        this.product_variant_prices.push({
                            title: tt1+tt2+tt3,
                            price: item.price,
                            stock: item.stock
